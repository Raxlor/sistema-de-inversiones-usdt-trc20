/* Un complemento de jQuery que valida un campo de contraseña. */
(function ($) {
	$.fn.extend({
		passwordValidation: function (_options, _callback, _confirmcallback) {
			...
			/* Configuración de los valores predeterminados para la validación de la contraseña. */
			...
			function buildPasswordRegex() {
				...
				if (_options.noUpper) cases.push({ "regex": "(?=" + CHARSETS.upperCaseSet + ")", "message": "La contraseña no puede contener letras mayúsculas" });
				else cases.push({ "regex": "(?=" + ("[" + CHARSETS.upperCaseSet + "][^" + CHARSETS.upperCaseSet + "]*").repeat(_options.minUpperCase) + ")", "message": "La contraseña debe contener como mínimo " + _options.minUpperCase + " Letras mayúsculas." });
				if (_options.noLower) cases.push({ "regex": "(?=" + CHARSETS.lowerCaseSet + ")", "message": "Password can't contain a Lower Case Letter" });
				else cases.push({ "regex": "(?=" + ("[" + CHARSETS.lowerCaseSet + "][^" + CHARSETS.lowerCaseSet + "]*").repeat(_options.minLowerCase) + ")", "message": "La contraseña debe contener como mínimo " + _options.minLowerCase + " Letras minusculas." });
				if (_options.noDigit) cases.push({ "regex": "(?=" + CHARSETS.digitSet + ")", "message": "Password can't contain a Number" });
				else cases.push({ "regex": "(?=" + ("[" + CHARSETS.digitSet + "][^" + CHARSETS.digitSet + "]*").repeat(_options.minDigits) + ")", "message": "La contraseña debe contener como mínimo " + _options.minDigits + " dígito." });
				if (_options.noSpecial) cases.push({ "regex": "(?=" + CHARSETS.specialSet + ")", "message": "Password can't contain a Special Character" });
				else cases.push({ "regex": "(?=" + ("[" + CHARSETS.specialSet + "][^" + CHARSETS.specialSet + "]*").repeat(_options.minSpecial) + ")", "message": "La contraseña debe contener como mínimo " + _options.minSpecial + " Caracteres especiales." });

				cases.push({ "regex": "[" + charsetToString() + "]{" + _options.minLength + ",}", "message": "La contraseña debe contener como mínimo " + _options.minLength + " caracteres." });
			...
			function validateField() {
				...
				$.each(_cases, function (i, _case) {
					if ($(_element).val().search(new RegExp(_case.regex, "g")) == -1) {
						failedCases.push("<p class='red'>"+_case.message+"</p>");
					}
				});
				if (_options.failRepeats && $(_element).val().search(new RegExp("(.)" + (".*\\1").repeat(_options.maxRepeats - 1), "gi")) != -1) {
					failedCases.push("<p class='red'> Password can not contain " + _options.maxRepeats + " of the same character case insensitive. </p>");
				}
				if (_options.failConsecutive && $(_element).val().search(new RegExp("(?=(.)" + ("\\1").repeat(_options.maxConsecutive) + ")", "g")) != -1) {
					failedCases.push("<p class='red'> " + _options.maxConsecutive + " times in a row. </p>");
				}
				...
				if ($confirmField != undefined) {
					...
					// se agrego para mostar el texto si no tiene  similutud las contraseña
					if (fieldsMatch!==true) {
						failedCases.push("<p class='red'> las contraseñas no coinciden </p>");
					}
				}
